...
// trace(W^T X H), (X: m*n, W: m*k row-major, H n*k row major)
...
class smat_t{
	...
	public:
		...
		void clear_space();
		void from_ptr(const smat_t* m){
			rows=m->rows;cols=m->cols;nnz=m->nnz;
			val = m->val; val_t = m->val_t;
			col_ptr = m->col_ptr; row_ptr = m->row_ptr;
			col_idx = m->col_idx; row_idx = m->row_idx;
			col_nnz = m->col_nnz; row_nnz = m->row_nnz;
			mem_alloc_by_me = false; read_from_binary = false;}
		smat_t deref(){smat_t ret; ret.from_ptr(this); return ret;}
		smat_t transpose();
		smat_t row_subset(const std::vector<int> &subset);
		smat_t row_subset(const int *subset, int subset_size);

		long nnz_of_row(size_t i) const {return (row_ptr[i+1]-row_ptr[i]);}
		long nnz_of_col(size_t i) const {return (col_ptr[i+1]-col_ptr[i]);}
